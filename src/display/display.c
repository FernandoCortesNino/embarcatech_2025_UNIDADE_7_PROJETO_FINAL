...
// Desenha a pontuação no display
...
// Desenha a vida no display
void draw_life(ssd1306_t *ssd, int life)
{
    char life_msg[20];
    snprintf(life_msg, sizeof(life_msg), "Vida: %d", life);

    // Atualiza o display
    ssd1306_fill(ssd, false);
    ssd1306_rect(ssd, 3, 3, 122, 58, true, false);
    ssd1306_draw_string(ssd, life_msg, 8, 48);
    ssd1306_send_data(ssd);
}
